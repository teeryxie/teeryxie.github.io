...
- `records`: each has `id`, `cohort`, `model`, optional `variant`, and `metrics: {who, when, qgold, qens, cov_plus, qens_joint}`. Optional `notes` and `source` identify row-specific qualifications. `status: "pending"` suppresses all scores.
